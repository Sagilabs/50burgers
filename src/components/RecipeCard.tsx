...
const RecipeCard: React.FC<RecipeCardProps> = ({ recipe, onToggleFavorite, onClick }) => {
  ...
  return (
    <div 
      className="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden transform transition duration-300 hover:shadow-lg hover:-translate-y-1 cursor-pointer"
      onClick={() => onClick(id)}
    >
      <div className="relative h-48 overflow-hidden">
        ...
        />
        <button 
          className="absolute top-2 right-2 p-2 bg-white dark:bg-gray-800 bg-opacity-70 rounded-full transition-colors hover:bg-opacity-100"
          onClick={(e) => {
            e.stopPropagation();
            onToggleFavorite(id);
          }}
      ...
      <div className="p-4">
        ...
        <div className="flex justify-between items-center text-sm">
          <div className="flex items-center">
            ...
            <span className="text-gray-700 dark:text-gray-300">{totalTime} min</span>
          </div>
          
          <div className="flex items-center">
            <ChefHat className="w-4 h-4 text-gray-500 dark:text-gray-400 mr-1" />
